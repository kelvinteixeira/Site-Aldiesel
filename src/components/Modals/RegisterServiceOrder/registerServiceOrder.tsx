...
import { RegisterOrderSchema } from "../../../utils/Yup/schema";
...
export function ModalRegisterServiceOrder(
  params: ModalRegisterServiceOrderProps
) {
    ...
    <Styled.Container>
      ...
      >
        ...
        <Formik
          ...
          validationSchema={RegisterOrderSchema}
          onSubmit={onSubmit}
        >
          {(props) => (
            <Form onSubmit={props.handleSubmit}>
              <Styled.CardContent>
                <Styled.Title>
                  {" "}
                  <Styled.FiToolStyled /> Informações iniciais
                </Styled.Title>
                ...
                <Styled.FormGroupStyled>
                  ...
                  <Field
                    as="textarea"
                    name="diagnosis"
                    className="form-control input textareasSimples"
                  />
                  <ErrorMessage name="diagnosis">
                    {(msg) => (
                      <Styled.MsgError>
                        diagnóstico é obrigátorio
                      </Styled.MsgError>
                    )}
                  </ErrorMessage>
                </Styled.FormGroupStyled>
                    ...
                    <div>
                      {props.values.dtcsInfo.map((dtcs, indexDtcs) => (
                        <Row className="mb-1" key={indexDtcs}>
                          <Col xs={2}>
                            <Styled.FieldStyled
                              name={`dtcsInfo.${indexDtcs}.code`}
                            />
                          </Col>
                          <Col>
                            <Styled.FieldStyled
                              name={`dtcsInfo.${indexDtcs}.dtc`}
                            />
                          </Col>
                          <Col xs={3}>
                            <Styled.FieldStyled
                              name={`dtcsInfo.${indexDtcs}.dtcState`}
                            />
                          </Col>
                        </Row>
                      ))}
                ...
                <Styled.Title>
                  <Styled.BsPinStyled /> Procedimentos{" "}
                </Styled.Title>
                    ...
                    <div>
                      {props.values.dtcsInfo.map((detalhes, indexDetalhes) => (
                        <Row className="mb-1" key={indexDetalhes}>
                          <Col>
                            <Styled.FieldStyled
                              name={`dtcsInfo.${indexDetalhes}.actions`}
                            />
                          </Col>
                        </Row>
                      ))}
      ...
      >
        ...
        <Modal.Title>
          {" "}
          <Styled.SubTitle>Cadastrando Ordem de serviço...</Styled.SubTitle>
        </Modal.Title>
      </Modal>
    </Styled.Container>
  );
